...
def wav_to_frequency_image(wav_file):
    ...
    # i could have used rfft(only positive) instead of fft but i like it more like this, still seems off?
    n = len(data)
    freqs = np.fft.fftfreq(n, 1 / samplerate)  # positive frequencies
    fft_magnitude = np.abs(np.fft.fft(data)) / n  # normalize -> amplitude
...
# Example usage
wav_to_frequency_image("squabble up.wav")
wav_to_spectrogram_image("squabble up.wav")

wav_to_frequency_image("paranoia.wav")
wav_to_spectrogram_image("paranoia.wav")
